#!/usr/bin/env python3
"""Render qeeg_bandratios_cli outputs into a self-contained HTML report.
...
Inputs:
  - bandratios.csv (required)
  - bandratios.json (optional; adds per-column descriptions/units)
  - bandratios_run_meta.json (optional; adds build/run metadata)

Typical usage:

  # Pass an output directory produced by qeeg_bandratios_cli
  python3 scripts/render_bandratios_report.py --input out_ratios

  # Or pass bandratios.csv directly
  python3 scripts/render_bandratios_report.py --input out_ratios/bandratios.csv --out report.html
...
def _guess_paths(inp: str, out: Optional[str]) -> Tuple[str, str, Optional[str], Optional[str]]:
    ...
    if _is_dir(inp):
        ...
        csv_path = os.path.join(out_dir, "bandratios.csv")
        sidecar = os.path.join(out_dir, "bandratios.json")
        run_meta = os.path.join(out_dir, "bandratios_run_meta.json")
        if out is None:
            out = os.path.join(out_dir, "bandratios_report.html")
    else:
    ...
    return csv_path, out or "bandratios_report.html", sidecar, run_meta
...
def _column_meta(sidecar: Optional[Dict[str, Any]], key: str) -> Tuple[str, str, str]:
    """Return (long_name, description, units) if present."""
    if not sidecar:
        return "", "", ""
    info = sidecar.get(key)
    if not isinstance(info, dict):
        return "", "", ""
    long_name = info.get("LongName", "") if isinstance(info.get("LongName"), str) else ""
    ...
    return long_name, desc, units


def _svg_bar_chart(labels: Sequence[str], values: Sequence[float], *, title: str, units: str) -> str:
    """Simple horizontal bar chart SVG.

    Bars are drawn around a zero axis so log10-transformed ratios (which may be
    negative) are displayed sensibly.
    """
    w, h = 820, 320
...
def _render_run_meta(run_meta: Optional[Dict[str, Any]]) -> str:
    ...
    # Add a bounded number of additional simple keys.
    extra = 0
    ...
    return (
        '<div class="card">'
        '<h2>Run metadata</h2>'
        '<div class="note">Build/run metadata written by <code>qeeg_bandratios_cli</code> (if available).</div>'
        '<table class="kv">'
        + "".join(rows)
        + "</table>"
        "</div>"
...
def _render_stats_table(
    ...
    if not num_cols:
        return '<div class="note">No numeric ratio columns detected.</div>'

    headers = ["metric", "n", "min", "median", "mean", "max", "units"]

    def td_num(v: float) -> str:
    ...
    for col in num_cols:
        ...
        _long, _desc, units = _column_meta(sidecar, col)

        body.append(
            "<tr>"
            f"<td><code>{_e(col)}</code></td>"
            f"<td data-num=\"{n}\">{n}</td>"
            f"<td data-num=\"{mn}\">{_e(td_num(mn))}</td>"
            f"<td data-num=\"{med}\">{_e(td_num(med))}</td>"
            f"<td data-num=\"{mean}\">{_e(td_num(mean))}</td>"
            f"<td data-num=\"{mx}\">{_e(td_num(mx))}</td>"
            f"<td>{_e(units)}</td>"
            "</tr>"
        )

    if not body:
        return '<div class="note">No finite values found in ratio columns.</div>'
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ap = argparse.ArgumentParser(description="Render bandratios.csv to a self-contained HTML report (stdlib only).")
    ap.add_argument("--input", required=True, help="Path to bandratios.csv, or the outdir containing it.")
    ap.add_argument("--out", default=None, help="Output HTML path (default: <outdir>/bandratios_report.html).")
    ap.add_argument(
        "--top-n",
        type=int,
        default=10,
        help="How many channels to show in the high/low charts for each ratio (default: 10).",
    )
    ap.add_argument("--title", default="Band ratios report", help="Report title.")
    ...
    if not os.path.exists(csv_path):
        raise SystemExit(f"Could not find bandratios.csv at: {csv_path}")
    ...
    for col in num_cols:
        ...
        low = vals_sorted[: min(top_n, len(vals_sorted))]
        high = list(reversed(vals_sorted))[: min(top_n, len(vals_sorted))]

        long_name, desc, units = _column_meta(sidecar, col)
        title_base = long_name or col
        subtitle = desc

        charts.append(
            '<div class="card">'
            f"<h3><code>{_e(col)}</code></h3>"
            + (f'<div class="note">{_e(subtitle)}</div>' if subtitle else '')
            + _svg_bar_chart(
                [k for k, _ in high],
                [v for _, v in high],
                title=f"{title_base}: highest {len(high)}",
                units=units,
            )
            + _svg_bar_chart(
                [k for k, _ in low],
                [v for _, v in low],
                title=f"{title_base}: lowest {len(low)}",
                units=units,
            )
            + "</div>"
        )
...
/* Charts */
.chart { width: 100%; height: auto; overflow: visible; margin-top: 8px; }
...
"""
    ...
    stats_html = _render_stats_table(channel_col, num_cols, rows, sidecar)
    ...
    charts_html = "".join(charts) if charts else '<div class="card"><div class="note">No numeric ratio columns found to chart.</div></div>'
...
<main>
  <div class=\"card\">
    <h2>About</h2>
    <div class=\"note\">
      This report visualizes <code>bandratios.csv</code> written by <code>qeeg_bandratios_cli</code>.
      It is for research/educational inspection only and is not a medical device.
      Click any column header to sort the tables.
    </div>
  </div>
  ...
  <div class=\"card\">
    ...
    <div class=\"note\">Computed from finite values across channels for each ratio column.</div>
    {stats_html}
  </div>

  <div class=\"card\">
    <h2>Band ratios table</h2>
    <div class=\"table-filter\">
      <div class=\"table-controls\">
        <input type=\"search\" placeholder=\"Filter rows…\" oninput=\"filterTable(this)\" />
        <button type=\"button\" onclick=\"downloadTableCSV(this, 'bandratios_table_filtered.csv', true)\">Download CSV</button>
  ...
  <div class=\"card\">
    <h2>High/low channels per ratio</h2>
    <div class=\"note\">Each ratio shows the highest and lowest channels (top-N) by value.</div>
  </div>
