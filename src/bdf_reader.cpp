...
static bool is_microvolt_dimension(std::string dim) {
  ...
  if (dim.find("µv") != std::string::npos) return true;
  if (dim.find("μv") != std::string::npos) return true;
...
static bool looks_like_exg_or_eeg_channel(const std::string& sanitized_label,
  ...
  if (starts_with(key, "eeg")) return true;
  if (starts_with(key, "ch")) return true;
...
EEGRecording BDFReader::read(const std::string& path) {
  ...
  int num_records = 0;
  try {
    num_records = to_int(num_records_s);
  } catch (...) {
    num_records = 0;
  }
  ...
  try {
    record_duration = to_double(record_duration_s);
  } catch (...) {
    record_duration = 0.0;
  }
  ...
  if (!(fs_target > 0.0)) throw std::runtime_error("BDF: invalid sampling rate");
  ...
  if (target_spr <= 0) throw std::runtime_error("BDF: failed to determine target samples_per_record");
  ...
  if (kept_names.empty()) {
    throw std::runtime_error("BDF: no signals found");
  }
  ...
  if (nsamp != target_total) {
    throw std::runtime_error("BDF: internal length mismatch (nsamp != target_total)");
  }
  for (const auto& ch : rec.data) {
    if (ch.size() != nsamp) {
      throw std::runtime_error("BDF: channel length mismatch after resample");
    }
  }
