...
int main() {
  ...
  // Add a couple of events so we round-trip BDF+ annotations.
  rec.events.push_back(AnnotationEvent{0.5, 0.0, "Start"});
  rec.events.push_back(AnnotationEvent{1.2, 0.3, "Task"});

  const std::filesystem::path out = std::filesystem::temp_directory_path() / "qeeg_test_bdf_writer_roundtrip.bdf";
  ...
  assert(std::fabs(rec2.fs_hz - rec.fs_hz) < 1e-6);

  // BDFReader should parse BDF+ "BDF Annotations" back into rec2.events.
